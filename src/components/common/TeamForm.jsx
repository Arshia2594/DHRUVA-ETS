    ...
    >
      ...
      >
        ...
        <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <SelectField label="Designation" name="Designation" value={formData.Designation} onChange={handleChange} options={["Project Engineer", "Software Tester", "Project Manager", "Team Lead"]} />

          <SelectField label="Role" name="Role" value={formData.Role} onChange={handleChange} options={["User", "Manager", "Admin"]} />
        ...
        <div className="flex justify-end gap-4">
          <button type="button" onClick={() => setIsCreateUpdate(false)} className="bg-gray-400 px-6 py-2 rounded-lg text-white">
            Cancel
          </button>

          <button type="submit" className="bg-green-700 px-6 py-2 rounded-lg text-white">
            {isEdit ? "Update" : "Save"}
          </button>
        </div>
      </motion.form>
...
const InputField = ({ label, name, value, onChange, type = "text" }) => (
  <div>
    ...
    <input name={name} value={value} onChange={onChange} type={type} className="border p-2 rounded-lg w-full dark:bg-gray-900" />
  </div>
);
...
const SelectField = ({ label, name, value, onChange, options }) => (
  <div>
    <label className="block font-medium mb-1">{label}</label>
    <select name={name} value={value} onChange={onChange} className="border p-2 rounded-lg w-full dark:bg-gray-900">
      <option value="">Select {label}</option>
      {options.map((opt) => (
